...
Return ONLY valid JSON, no markdown.
  ...
  try {
    ...
    console.error("Gemini generateSpendingReport error:", error);
...
In 3-4 friendly sentences, explain why the score is ${score}, what they're doing well, and the top 2 things they can do to improve it. Be encouraging and specific.
  ...
  try {
    ...
    console.error("Gemini explainHealthScore error:", error);
    throw new Error(`Failed to explain financial health score: ${error.message}`);
  }
};
...
Return ONLY valid JSON, no markdown.
  ...
  try {
    ...
    console.error("Gemini generateGoalPlan error:", error);
    throw new Error(`Failed to generate saving plan for goal: ${error.message}`);
  }
};
...
const parseVoiceExpense = async (audioBuffer, language = "en-IN") => {
  try {
    ...
    // Step 2: Use Gemini to parse the transcript into structured expense data
    const parsePrompt = `
...
Category must be one of: Food & Dining, Shopping, Transportation, Entertainment, Healthcare, Utilities, Other.
    ...
    const parsedText = parseResponse.response.text().trim();
